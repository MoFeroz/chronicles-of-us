    ...
    <div className="flex flex-col h-full animate-fade-in pb-20">
      ...
      <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-6">
        ...
        <div className="bg-slate-800/50 p-6 rounded-lg border border-slate-600 shadow-lg">
          <p className="font-story text-lg leading-relaxed text-amber-50">
            {node.text}
          </p>
        </div>
          ...
          <div className="mt-auto flex flex-col gap-4">
             ...
             {node.options.map((opt) => (
                <button 
                  ...
                  className={`w-full font-pixel py-4 rounded-lg shadow-lg border-2 transition-transform active:scale-95 text-sm leading-relaxed px-2 
                    ${opt.type === 'HEROIC' 
        ...
        {step === 'RESULT' && outcomeData && (
          <div className="flex flex-col gap-4 animate-slide-up">
            ...
            {!isSuccess && selectedOption && (
              <div className="p-3 bg-red-900/30 border-l-4 border-red-500 text-red-100 italic text-sm font-story">
            ...
            <div className={`p-4 rounded-lg border-2 ${isSuccess ? 'border-green-500/50 bg-green-900/20' : 'border-yellow-500/50 bg-yellow-900/20'}`}>
              <p className="font-story text-lg italic text-white">
                "{outcomeData.text}"
              </p>
            </div>

            {/* Media Display */}
            <div className="rounded-xl overflow-hidden shadow-2xl border-4 border-slate-700 bg-black">
            ...
            <button 
              onClick={onNext}
              className="w-full bg-yellow-600 hover:bg-yellow-500 text-slate-900 font-pixel py-4 rounded-lg shadow-lg mt-4"
            >
              CONTINUE JOURNEY
            </button>
          </div>
